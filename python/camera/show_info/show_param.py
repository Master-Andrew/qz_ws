...
def showGNSSIMUExtrinsic(gnss_imu_extrinisc):
  ...
  imu_yaw = float(gnss_imu_extrinisc["imu_extrinsics"]["yaw"])
  imu_pitch = float(gnss_imu_extrinisc["imu_extrinsics"]["pitch"])
  imu_roll = float(gnss_imu_extrinisc["imu_extrinsics"]["roll"])
...
def obcCameraExtrinsic(obc_extrinsic_list):
  ...
  for obc_extrinsic in obc_extrinsic_list:
    if "camera_id" not in obc_extrinsic["obc_config"]["node_name_to_camera_id"]:
      continue
    if type(obc_extrinsic["obc_config"]["node_name_to_camera_id"]["camera_id"]) != list:
      obc_extrinsic["obc_config"]["node_name_to_camera_id"]["camera_id"] = [
        obc_extrinsic["obc_config"]["node_name_to_camera_id"]["camera_id"], ]
...
def showVechicleExtrinsic(car_number):
  ...
  with open(param_file, "r") as f:
    ...
    for camera_extrinsic in camera_extrinsic_list:
      if "ref_lidar_id" not in camera_extrinsic:
        # for i in range(5):
        #   print("!" * 100)
        # print(camera_extrinsic["sn"],
        #       ", can not findref_lidar_id!!!")
        # for i in range(5):
        #   print("!" * 100)
        # continue
        camera_extrinsic["ref_lidar_id"] = "NOT_DEFINE"
        showCameraExtrinsic(camera_extrinsic, [],
                            obc_camera_extrinsic)

      elif camera_extrinsic["ref_lidar_id"] not in lidar_extrinsic_dict:
